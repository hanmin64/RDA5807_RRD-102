...
void RDA5807::getStatusRegisters()
{
    ...
    // 修復 (ESP32 core 3.x)：requestFrom() 已完整送出 STOP 並釋放匯流排，
    // 後續不得再呼叫 Wire.endTransmission()；否則 ESP32 core 3.x 的
    // TwoWire::endTransmission(true) 會用上一次交易殘留的 txAddress/txBuffer
    // 再發一次幽靈 I2C 寫入 (常寫到 SSD1306 0x3C)，破壞 OLED 定址狀態，
    // 造成顯示畫面左上角「由左到右漸層掃描」殘影。
