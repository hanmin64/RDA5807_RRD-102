 ...
 * - OpenWeatherMap 天氣查詢：每 30 秒更新，圖形化天氣圖示
 * - OLED 雙頁輪播：時鐘頁 + 天氣頁，FM 頻率持續顯示
...
#include <WiFi.h>             // ESP32 WiFi 函式庫，用於網路連線
#include "WiFiManager/WiFiManager.h" // WiFi 配網模組
...
void setup() {
  ...
  if (WiFiMgr.isConnected()) {
    NTP.begin();
    Weather.apiKey = "請輸入您的 OpenWeatherMap API key";
    Weather.fetch();
  }
...
void loop() {
  ...
  WiFiMgr.handle();               // 斷線自動重連 + 重置鍵 (GPIO0) 長按清除設定
  if (WiFiMgr.isConnected()) {
    NTP.handle();                 // 維持 NTP 時鐘運作，每小時自動校時
    static unsigned long lastWeather = 0;
    if (millis() - lastWeather > WEATHER_INTERVAL_MS) {
      lastWeather = millis();
      Weather.fetch();            // 每 30 秒更新天氣資料
    }
  }

  // 3. 檢查旋鈕焦點是否超時，超時則釋放焦點 (避免死佔)
  updateKnobFocus();

  // 4. 處理音量旋鈕 (所有模式下都持續偵測音量變化)
  processVolume();
...
void updateDisplay() {
  ...
  // 分兩頁輪播：Page 0 = 時鐘, Page 1 = 天氣, 每 ~6 秒切換
  {
    ...
    static bool showClockPage = true;
...
void drawWeatherIcon(int x, int y, int code) {
  // code 分類：
  //   2xx = 雷雨, 3xx = 毛毛雨, 5xx = 雨
  //   6xx = 雪, 7xx = 霧/霾, 800 = 晴, 801~804 = 雲
  if (code == 800) {
    // --- 晴天：大圓形 + 8 條射線 ---
    ...
    for (int a = 0; a < 360; a += 45) {
      ...
      int x1 = x + 16 + cos(rad) * 11;
      int y1 = y + 16 + sin(rad) * 11;
      int x2 = x + 16 + cos(rad) * 20;
    ...
    display.fillCircle(x + 22, y + 16, 9, SSD1306_WHITE);
    display.fillCircle(x + 16, y + 11, 9, SSD1306_WHITE);
    display.drawLine(x + 18, y + 24, x + 12, y + 31, SSD1306_WHITE);
    display.drawLine(x + 12, y + 31, x + 20, y + 31, SSD1306_WHITE);
    display.drawLine(x + 20, y + 31, x + 16, y + 28, SSD1306_WHITE);
    ...
    display.drawPixel(x + 24, y + 28, SSD1306_WHITE);
    display.drawPixel(x + 24, y + 29, SSD1306_WHITE);
    display.drawLine(x + 8, y + 27, x + 8, y + 30, SSD1306_WHITE);
    display.drawLine(x + 16, y + 28, x + 16, y + 31, SSD1306_WHITE);
    display.drawLine(x + 24, y + 27, x + 24, y + 30, SSD1306_WHITE);
  } else {
    // --- 霧/霾 或其他：水平波狀線 ---
    for (int i = 0; i < 3; i++) {
      int waveY = y + 8 + i * 10;
      for (int px = 0; px < 28; px += 4) {
        display.drawPixel(x + 2 + px + ((i & 1) * 2), waveY, SSD1306_WHITE);
      }
    }
  }
}
